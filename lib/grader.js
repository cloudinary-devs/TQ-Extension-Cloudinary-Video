...
module.exports = class Grader extends EventEmitter {
    ...
    /**
     *  DEV - use the working example answer for grading if it's provided in the key so we don't have to keep filling it out
     * @type {boolean}
     */
    cheat = true;
    /**
    ...
    constructor(helper, answerKey, passCallback, failCallback) {
        ...
        if (this.cheat) {//uses valid answers from the key for any that aren't provided
    ...
    applyAnswerKey() {
        ...
        for (const answerIndex in this.answers) { //answerIndex likely = "answer1","answer2" or "url1","url2"
            let self = this;
     ...
     * @param answerIndex "answer1", "answer2", "url1", "url2", etc
    ...
    getVideoUrl(answerIndex) {
        ...
        if (fs.existsSync(filePath)) {
            ...
            console.log('cached file does not exist, using answer url to re-request file.');
