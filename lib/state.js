...
module.exports = new class State {
     ...
     */
    setCheatMode(turnOn){
        this.data.set('dev.cheatMode',Boolean(turnOn)).write();
    }

    getCheatMode(){
        return Boolean(this.data.get('dev.cheatMode').value());
    }

    /**
     *
     * @returns {boolean} cheat mode is now on (true) or off (false)
     */
    toggleCheatMode(){
        this.setCheatMode(!this.getCheatMode());
        return this.getCheatMode();
    }
